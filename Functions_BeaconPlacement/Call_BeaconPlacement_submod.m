function Call_BeaconPlacement_submod(FloorPlanPath, BeacPlacementName)
        ...
        for i = 1:size(CornerInLosInd,1) 
            ...
            if(COMPARE_METRIC == 'U')
                ...
                for w=1:size(StoreClass,2)
                    ...
                    U_D = find(StoreClass(:,w)<=3);
                    V1_D = find(StoreClass(:,w)<=5);
                    V2_D = find(StoreClass(:,w)<=4);
                    SizeUL(w) = length(U_D) + 2*length(V1_D) + length(V2_D);
